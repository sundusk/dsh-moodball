...
- **双击**：小球左右快速晃动约 1.5 秒（幅度衰减），表示兴奋
- **锁定位置**（设置面板 → 行为）开启后不可拖拽，仍可双击

### 状态展示（截图用）

菜单栏 →「状态展示…」（⌘D）：选择一个状态，面板实时展示真实渲染的小球
（外发光 + 渐变 + 眼睛），可切换显示/隐藏气泡文字。
点「**保存 PNG**」会把当前状态导出为 **512×512 透明背景 PNG**（存到桌面，
文件名 `moodball-<状态>.png`）——所有状态导出尺寸完全一致，可直接用于文档。
...
所有颜色都可在设置面板自定义。球图由「状态展示」面板导出的 **512×512 原图**（真实渲染，正常尺寸显示）；
`docs/generate-moodball-svgs.sh` 保留作纯球 SVG 后备。
...
- **「设置 → 插件」里怎么没有心情球插件卡片？** 这是设计如此：`dsh-moodball-status` 是纯 host
  状态插件，没有任何设置项（所有配置都在 app 自己的设置面板里），所以不会出现在插件配置卡片
  列表中。它显示在「设置 → 插件 → **插件列表**」标签页（只读的已挂载插件清单，状态为「已挂载」）。
